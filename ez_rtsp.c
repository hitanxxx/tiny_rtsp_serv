...
int ezrtsp_video_paramset_save( int chn, struct video_stream * stream )
{
    ...
    if(chn == 1) {
        ...
        nal_start = nalu_find_start( p, end );
        for(;;) {
            //int nlen = 0;
            ...
            nal_end = nalu_find_start(nal_start, end);
            //nlen = nal_end - nal_start;
            naluint_cntn ++;
            nal_start = nal_end;
        }
        dbg("frame naluint number [%d]\n", naluint_cntn );
    }
#endif
#endif
...
int ezrtp_packet_build( rtsp_con_t * c, int payload, const char * data, int size, int marker, unsigned long long ts )
{
...
#if(1)
    static unsigned long long rtp_cnt = 0;
    rtp_cnt ++;
    // dbg("rtp packet cnt [%lld]\n", rtp_cnt );
#endif
    ...
    rtp_hdr[0] = 2 << 6;   // fix 
    rtp_hdr[1] = (payload &0x7f) | ((marker&0x01)<<7); // fix
    PUT_16( rtp_hdr + 2, c->rtp_seq );
    c->rtp_seq += 1;

    PUT_32( rtp_hdr + 4, c->rtp_ts );
    if( marker ) {
        c->rtp_ts += (90000/video_get_fps(1))/2;
    }

    PUT_32( rtp_hdr + 8, 0x252525 );
    send_len += 12;
...
static void * ezrtp_send_task( void * para )
{
    ...
    while( c->rtp_stat == 1 ) {
        ...
        sys_cache_frm_t * frm = sys_cache_frm_get( 1, chn_seq );
        if( frm ) {
            ezrtp_send_frame( c, (char*)frm->data, frm->datan, ts );
            ts += (90000/15);
...
void ezrtsp_resp( evt_obj_t * ev, int trigger_type )
{
    ...
    if( c->req_method ==  METHOD_OPTIONS ) {
        ...
        if( c->req_setup_tcp == 0 ) {
            /// if not use TCP mode, return 461 not support
            /// normaly, rtsp player will be request tcp again
            c->last += snprintf( c->buffer, sizeof(c->buffer),
    ...
    //return ezrtsp_resp_send( ev, EV_W );
}
...
void ezrtsp_process_request( evt_obj_t * ev, int trigger_type )
{
	...
	while( !c->req_complete ) {
		...
		if( recvd <= 0 ) {
			...
			if( (errno == EAGAIN) || (errno == EWOULDBLOCK) ) {
                // dbg("process request addr [%p]\n", c );
                if( c->rtp_stat == 1 ) {
                    evt_timer_del( ev );
                    dbg("recv again. con in playing, do nothing\n");
                    /// reset the meta
                    memset( c->buffer, 0, sizeof(c->buffer) );
                    c->start = c->pos = c->last = c->buffer;
                    c->end = c->start + sizeof(c->buffer);
    ...
    if( strstr ( c->pos, "OPTIONS " ) != NULL ) {
        ...
        dbg("rtsp con req method [PLAY]\n");
        c->req_method = METHOD_PLAY;
    } else {
        err("not support req method\n");
        return ezrtsp_con_free( c );
    }
    
    evt_opt( ev->evt, c->fd, (void*)c, ezrtsp_resp, EV_W );
    //return ezrtsp_resp( ev, EV_W );
}
