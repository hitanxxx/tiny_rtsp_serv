...
static unsigned const g_freq_tbl[16] = {
...
static unsigned char g_adts_header[4] = {0};
static int g_as_stat = 0;
    ...
    while(sendn < datan) {
        ...
        if(rc < 0) {
            if((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
                ...
                tryn++;
                if(tryn >= 7) 
                    return -1;///drop the packet if failure times more than 7
                continue;
            }
...
void ezrtsp_response(ev_ctx_t * ctx, int fd, void * user_data, int op)
{
    ...
    if(c->imethod == METHOD_OPTIONS) {
        ...
        if(c->faudioenb) {
            #if(0)
            ///if have't adts header, mabey use this
            ...
            char config[32] = {0};
            unsigned char profile = (g_adts_header[2]&0xc0)>>6;
            unsigned char freq = (g_adts_header[2]&0x3c)>>2;
            unsigned char channel = ((g_adts_header[2]&0x1)<<2)|((g_adts_header[3]&0xc0)>>6);
        ...
        if(c->fvideoenb) {
            rtpinfon += snprintf(rtpinfo + rtpinfon, sizeof(rtpinfo) - rtpinfon, 
                "url=rtsp://%s:554/%s/track=0;seq=0;rtptime=0",
                wireless_ip,
        ...
        if(c->faudioenb) {
            rtpinfon += snprintf(rtpinfo + rtpinfon, sizeof(rtpinfo) - rtpinfon, 
                "%surl=rtsp://%s:554/%s/track=1;seq=1;rtptime=0",
                c->fvideoenb ? "," : "",
                wireless_ip,
                (c->ichn==0 ? "main_ch" : "sub_ch")
            );
                ...
                for(i = 0; i < 8; i++) {
                    ...
                    if(c->fuse && c->fplay && (c->ichn == ch)) {
                        if(frm->typ == 0 && c->faudioenb) {
                            ...
                            c->session_audio.ts += 8000/(8000.0f/1024.0f);
                        }

                        if(frm->typ > 0 && c->fvideoenb) {
